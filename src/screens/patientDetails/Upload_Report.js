...
const Upload_Report = ({ data }) => {
    ...
    const renderItem = (item) => {
        ...
        const imageId =
            item.uploadedReports
                ?.[0]?.images?.[0]?.id ?? null;
        const pdfurl = item.uploadedReports?.map(report => report.id);
    
        

        const imageUrl = imageId
            ? `https://beta.hru.today/report/${item._id}/${imageId}/display.image`
            : null;
            ...
            <View style={styles.card}>
                <View style={styles.cardContent}>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            const url = `https://beta.hru.today/doctor/${item._id}/${pdfurl}/report.pdf`;
                            setSelectedPdf(url);
