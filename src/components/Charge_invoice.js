...
import { notifyPatient, refreshWorkQueueData, saveChargeInvoiceDetails } from '../api/authService';
import PdfViewer from './PdfViewer';



const Charge_invoice = ({ visible, onClose }) => {
  const { checkinData } = useContext(AppContext);

  const [isHRUSelected, setIsHRUSelected] = useState(false);
  const serviesaddation = checkinData?.invoice?.services
    ...
    setIsChecked((prev) => {
      ...
      if (newCheckedState) {
        ...
        const updatedRows = rows.map((row) => {
          ...
          const totaldues = parseFloat(row.totalDue || 0)
  ...
  const renderRow = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        <Text style={styles.payment_status}>
          {item.paymentCompleted ? <View
            style={{
        ...
        <View style={styles.field}>
          ...
          <TextInput
            style={[styles.input, styles.non_editable]}
        ...
        <View style={styles.field}>
          ...
          <TextInput
            ...
            editable={!item.paymentCompleted}
          />
        </View>
        ...
        <View style={styles.field}>
          ...
          <TextInput
            ...
            onChangeText={(value) => {
              ...
              updateRow(item.id, "onlinePayment", value)
            }
            }
            editable={!item.paymentCompleted}
          />
        ...
        <View style={styles.buttons}>
          ...
          {!item.isServiceMatched && item.id !== 1 && (
  ...
  const handlePaymentModeChange = (value) => {
    ...
    // Apply payment mode to all rows
    setRows((prevRows) =>
      prevRows.map((row) => ({
        ...row,
        modeOfPayment: value,
  ...
  const hanndleClick = async () => {
    ...
    for (let row of rows) {
      ...
      if (paymentCompleted) {
        servicies.push({
          ...
          onlinePayment: parseFloat(row.onlinePayment),
    ...
    const credentials = {
      ...
      "baseUrl": "https://beta.hru.today/",
      "mobileNo": checkinData.mobileNumber,
      "confirmTransaction": true,
      "services": servicies,
      "totalAmt": remainingBalance.totalDue,
      "wpMsg": false,
      "doctorFormatPrescription": false,
      "useDoctorWhatsApp": false
    }
    ...
    try {
      const response = await saveChargeInvoiceDetails(credentials)
  ...
  const handleRefresh = (async () => {
    ...
    try {
      ...
      if (response.doc?.serviceInfo?.rzrpStatus === "received") {
        setIsSubmitDisabled(false);
      ...
      <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>Charge & Invoice : {checkinData.firstName + " " + checkinData.lastName}</Text>
          ...
          <View style={styles.my_component_container}>
            <View style={styles.my_component_row}>
              ...
              <Text style={styles.my_component_text}>Paid Amount: ₹{isChecked ? remainingBalance.paidAmount : remainingBalance.paidAmount}</Text>
            ...
            <TouchableOpacity onPress={() => {
              ...
              setSelectedPdf(url);
            }}><Text style={[styles.my_component_text, { color: 'blue' }]}>Generate & Print Invoice</Text>
            </TouchableOpacity>


            {rows.some(row => !row.paymentCompleted) && (
              <>
                    ...
                    {(() => {
                      if (isChecked) {
                        if (remainingBalance.paidAmount > 0) {
                          ...
                          return `I confirm the receipt of complete amount of ₹${remainingBalance.charges.toFixed(2)}`;
                        }
                      } else {
                        if (remainingBalance.totalDue > 0 && rows.some(row => row.paymentCompleted)) {
                          let totalDueWithExtraRows = remainingBalance.totalDue;
                          return `I confirm the receipt of ₹${totalDueWithExtraRows.toFixed(2)}`;
                        } else if (remainingBalance.paidAmount > 0) {
            ...
            <View style={styles.my_component_button_container}>
              {/* <TouchableOpacity style={styles.my_component_button} onPress={hanndleClick}>
                <Text style={styles.my_component_button_text}>OK</Text>
              </TouchableOpacity> */}
              <TouchableOpacity
                style={[
              ...
              <TouchableOpacity style={[styles.my_component_button, styles.my_component_cancel_button]}>
